...
contract PlatziPunksDNA {
    ...
    function deterministicPseudoRandomDNA(uint256 _tokenId, address _minter) public pure returns (uint256) {
        ...
        // Convert the pararms added into bytes
        bytes memory encodedParams = abi.encodePacked(combinedParams);
        
        // Hashes the convertion and returns it
    ...
    // Number to get attributes
    uint8 constant ADN_SECTION_SIZE = 2;

    // Receives the dna and the part to discard of that and returns the two number of DNA
    ...
    // Functions to get the attributes of the platzipunk
